...
EOT
	 ...
	 if $npts-removed > 0 { 
		...
		my $msg = "fit with \x[03C7]\x[00B2]~ndf (ndf: Num. degrees freedom) and {$npts-removed} points removed";
 		say qq:to/EOT/ unless $quiet;
...
EOT
		...
		if $reduced-chi2 { 
			$TXT = $reset-parameters-std($TXT) if $reduced-chi2;
		 	my $msg = "fit with \x[for a03C7]\x[00B2]~ndf (ndf: Num. degrees freedom)";
 			say qq:to/EOT/ unless $quiet;
